...
unsigned char MQTT_FillBuf(char *buf)
{
	...
	if(Data_init.hand_id_copy !=0){
		memset(text, 0, sizeof(text));
		sprintf(text, "\"face_id\":\"%d\",", Data_init.hand_id_copy); // Temp是数据流的一个名称，temper是温度值
		strcat(buf, text);
	}
	// 刷卡
	if(ucArray_ID [ 0 ] !=0 ){
		...
		sprintf(text, "\"rfid\":\"%02X%02X%02X%02X\",", ucArray_ID [ 0 ],
										ucArray_ID [ 1 ],
										ucArray_ID [ 2 ],
										ucArray_ID [ 3 ]); // Temp是数据流的一个名称，temper是温度值
		strcat(buf, text);
	}
	// 密码
	if( device_state_init.GetPwd !=0 ){
		memset(text, 0, sizeof(text));
		sprintf(text, "\"pwd\":\"%d\",", device_state_init.GetPwd); // Temp是数据流的一个名称，temper是温度值
		strcat(buf, text);
	}
	
	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"door\":\"%d\",", device_state_init.open); // Temp是数据流的一个名称，temper是温度值
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"door_time\":\"%d\"", device_state_init.Door_Time); // Temp是数据流的一个名称，temper是温度值
	strcat(buf, text);
...
unsigned char MQTT_fid_Info(char *buf)
{
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"fid\":\"%d\"", Data_init.hand_id); // Temp是数据流的一个名称，temper是温度值
	strcat(buf, text);
